...
fprintf('The number of samples in Training set: \n')
tb1 = countEachLabel(imdsTrainingSet)
fprintf('The number of samples in Test set: \n')
tb2 = countEachLabel(imdsTestSet)
%%
...
for i=1 : num
      ...
      stats = ConfusionMat_MultiClass(cmat, 1);
...
%% ********************* plot ROc curve *****************
             ...
             %  plot(X,Y)
             %  xlabel('1-specificity'); ylabel('sensitivity');xlim([0 1]);ylim([0 1]);
             %  title(['ROC analysis (AUC: ' num2str(AUCpr) ')'])
